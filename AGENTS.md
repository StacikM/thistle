...
`ctest` runs the headless suites — `net_smoketest` (networking), `scene_smoketest` (Node save/load round-trip), and the 3D ones: `collision_smoketest`, `math3d_smoketest`, `mesh3d_smoketest`, `model_smoketest`, `voxel_smoketest`, `character_smoketest`, `particles_smoketest`, `terrain_smoketest`, `audio3d_smoketest`, `voxelnet_smoketest`, `animation_smoketest`, `scene3d_smoketest` (the Thistle Editor's level format), `cli_smoketest` (the `thistle` CLI's `new`/`init`, which the editor's projects screen runs, and the handover to the game at the end of `thistle run`; Python, skipped without it), `physics3d_smoketest` and `destruction_smoketest` (the last two each check the Jolt build or the stand-in, whichever `THISTLE_PHYSICS3D` picked), `server_smoketest` (`DedicatedServer`, linked against the headless `thistle_server`; see docs/dedicated-servers.md) — all real assertions, not smoke in the "did it not crash" sense. `-DTHISTLE_SERVER_ONLY=ON` builds only `thistle_server` and runs the headless suites that need nothing more against it (CI does this in a bare Ubuntu container, and checks nothing graphical got linked). `thistle_smoketest` (window + 2D/3D drawing + HTTP + IAP + post-processing) and `thistle_crash_smoketest` (deliberately crashes) are built by CI but not executed there — run those yourself when you touch anything they exercise. See `docs/building.md` for the full per-platform build story, including the specific MinGW/MSVC gotchas already found and fixed.
...
- `src/net.cpp`, `src/save.cpp`, `src/log.cpp`, `src/collision3d.cpp`, `src/three_models.cpp` — the parts both libraries build: the full engine (`thistle`) and the headless one for dedicated servers (`thistle_server`, no window/graphics/audio). Their internal glue is `src/thistle_core.h`, which must never need sokol; `src/thistle_internal.h` adds the GPU side. `src/headless.cpp` is what `thistle_server` has instead of `thistle.cpp` (plain file reading, textures as empty handles), and `src/dedicated_server.cpp` + `src/server_console.cpp` are `DedicatedServer` (see docs/dedicated-servers.md). A `World::draw` for something the server library also builds sits behind `#if !defined(THISTLE_SERVER)`.
...
- `tools/thistle-cli` — the `thistle` command-line tool for scaffolding/building/running a game project. See `docs/cli.md`. The editor runs it too (New project, Make it a Thistle project, Build & run), so it stays the one place a project's shape is decided.
- `tools/thistle-editor` — the Thistle Editor, a 3D level editor for `three::Scene3D` files, drawn with the engine's own 2D API. `main.cpp` is the editor, `hub.cpp` its projects screen and Build & run, `platform.cpp` running the CLI as a child process (POSIX and Windows). See its README for what's been verified by clicking through it.
...
- **Test your environment's defaults, not just the code.** A cloud container that worked on this repo set `PYTHONUNBUFFERED=1`, which hid a real bug: the CLI's output into a pipe was held back and lost, so the editor's Build & run never saw the game start (see `thistle run` in `docs/cli.md`). The owner found it on Windows. Run anything that reads another program's output with the settings a user has (`env -u PYTHONUNBUFFERED ...`).
