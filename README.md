...
- **3D (`thistle::three`):** a lit renderer with models from glTF/GLB and OBJ, sun shadows, point and spot lights, sky and fog, instancing, particles, terrain, skeletal animation, block worlds (Minecraft- or Teardown-style, endless or not, MagicaVoxel import), 3D sound, and a built-in character controller. Run on Windows with a real GPU (D3D11) and on Linux's software renderer; not yet on a Mac — see [docs/3d.md](docs/3d.md).
