...
- **`thistle build [--release]`** — configures on first run (creates `build/`), then just builds on every call after, on all CPU cores (`--parallel`: without it Make compiled one file at a time, and the first build, which compiles the engine, took several times longer). Debug by default.
- **`thistle run [--release]`** — builds, then runs the resulting binary in its place (`exec`). Finds it under whatever your platform's generator actually produced (`.app` bundle on macOS, `Debug/`/`Release/` subfolder on Windows' multi-config generator, a plain binary on Linux) so you never have to know that path yourself. It runs it from that folder, where the build copied `assets/` (the way a player's copy runs), so it works from anywhere inside the project, `src/` included. Windows has no real `exec` (Python's starts the program and exits at once, so a terminal got its prompt back while the game was still running, and the game's exit code was lost), so there it runs the game as a child, waits for it, and exits with the game's exit code.
