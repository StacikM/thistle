...
- **`thistle new <name> [--at PATH] [--template KIND]`** — scaffold a project. Default location is next to the engine (matching the existing convention every project in this repo already uses); `--at` overrides it. `--template` picks what it starts as:
  ...
  - `multiplayer`: a game and its dedicated server. Players run around an arena and see each other; the server (`src/server.cpp`, no window) gives each a spawn point, checks their moves, and takes commands typed into it (`list`, `kick 2 being rude`, ...). The game opens on a connect screen that shows why a connection failed or ended. What they share is `src/shared.hpp`. See [dedicated-servers.md](dedicated-servers.md).
  ...
  `--with MODULE` (repeatable) turns an optional engine module on from the start, the same as `thistle enable` afterwards: `thistle new mygame --template fps --with physics3d`. The template is recorded in `thistle.json` (`"template": "fps"`), which is how the Thistle Editor's project list shows what each project started as.

  The editor's New project form runs this command, so a project made there is the same as one made here.
- **`thistle init [--name NAME]`** — makes the current folder a Thistle project. It adds what `thistle new` would have made (`thistle.json`, `CMakeLists.txt`, `src/main.cpp`, `src/version.hpp.in`, `README.md`, `.gitignore`, `assets/`), but only what's missing: a file that's already there is never overwritten, and it lists what it added and what it kept. The name defaults to the folder's, made valid (`My Game!` → `My-Game`). If the folder already had its own `CMakeLists.txt`, that one is kept, and it says how to add the engine to it. It refuses a folder that already has a `thistle.json`. The editor's "Make it a Thistle project" runs this.
- **`thistle build [--release] [--server]`** — configures on first run (creates `build/`), then just builds on every call after, on all CPU cores (`--parallel`: without it Make compiled one file at a time, and the first build, which compiles the engine, took several times longer). Debug by default.
- **`thistle run [--release] [--server] [-- ARGS]`** — builds, then runs the resulting binary in its place (`exec`), with anything after `--` as its arguments. Finds it under whatever your platform's generator actually produced (`.app` bundle on macOS, `Debug/`/`Release/` subfolder on Windows' multi-config generator, a plain binary on Linux) so you never have to know that path yourself. It runs it from that folder, where the build copied `assets/` (the way a player's copy runs), so it works from anywhere inside the project, `src/` included. Windows has no real `exec` (Python's starts the program and exits at once, so a terminal got its prompt back while the game was still running, and the game's exit code was lost), so there it runs the game as a child, waits for it, and exits with the game's exit code.

  Just before starting the game it prints `$ <the game's path>`: the Thistle Editor's Build & run reads that line to know the build is done and the game is running. When its output goes into a pipe, the CLI writes it a line at a time and flushes before handing over. Before, Python held the line back and `exec` threw it away, so the editor kept saying "Building" while the game ran, and "The build failed" when it closed. Found on the owner's Windows 11 PC. It hadn't shown in the container these docs were tested in, because that environment sets `PYTHONUNBUFFERED=1`; `cli_smoketest` now checks the handover without it.
- **`--server`** (build and run) — a project's dedicated server: `src/server.cpp`, built as `<name>-server` on the headless engine (the `multiplayer` template has one; any project gets one by adding the file). `thistle build` builds it along with the game; `thistle build --server` builds only it, in `build-server/` with `-DTHISTLE_SERVER_ONLY=ON`, so it works on a machine with no graphics libraries. `thistle run --server -- --port 47001` builds it and runs it from its folder, where its `server.json`, `logs/` and save data go. A project without `src/server.cpp` is told so; one made by an older `thistle new` is told which lines its `CMakeLists.txt` needs. See [dedicated-servers.md](dedicated-servers.md).
...
- **`thistle editor run [project]`** — builds (if needed) and runs the editor in its place (on Windows, as a child it waits for, like `thistle run`), without touching your `PATH`. It edits the project folder you name, or else the project you're in (the nearest folder up with a `thistle.json`). The installed `thistle-editor` takes the same optional folder argument.
