...
- **Gamepad and haptics.** Same gap as Windows/Linux — both are Apple-only today (`#if defined(__APPLE__)` gates in `thistle.cpp`), so Android silently gets the no-op branch. Not Android-specific, just not done anywhere outside Apple's GameController.framework yet.
